...
-- Migration: Fix force_refund_credits to ensure description is updated
-- This ensures that even if the UI doesn't group correctly, the usage line says "Refunded"
...
BEGIN
  ...
  -- 1. Attempt Release Reservation (Restored from previous version)
  ...
  -- CRITICAL FIX: Update the original usage transaction description to reflect the refund status
  -- This ensures "Recent Activity" shows "Refunded: ..." instead of just "Scene image generation"
  -- if the UI fails to group them or if the user looks at raw data.
