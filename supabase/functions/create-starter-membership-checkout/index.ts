...
serve(async (req: Request) => {
  ...
  const monthlyPriceId = Deno.env.get("STRIPE_PRICE_STARTER_ID");
  const annualPriceId = Deno.env.get("STRIPE_PRICE_STARTER_ANNUAL_ID");
  ...
  if (!monthlyPriceId) missing.push("STRIPE_PRICE_STARTER_ID");
  if (interval === "year" && !annualPriceId) missing.push("STRIPE_PRICE_STARTER_ANNUAL_ID");
